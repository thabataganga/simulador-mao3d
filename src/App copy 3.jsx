...
const THUMB_BASE_RATIO = { xL: 24 / 70, yT: -2 / 14, zW: 28 / 55 };
const RATIOS = {
...
// ─── UI components ────────────────────────────────────────────────────────────
function LabeledSlider({ label, min, max, step=1, value, onChange, leftHint, rightHint, disabled, onHighlight }) {
  ...
  const commit = () => {
    ...
    setTemp(String(v)); onChange(v); onHighlight?.();
  };
  return (
    <div className="mb-3">
      ...
      <input type="range" min={min} max={max} step={step} value={value}
        onChange={e => { onChange(Number(e.target.value)); onHighlight?.(); }}
        disabled={disabled} className="w-full disabled:opacity-60" />
      {(leftHint||rightHint) && (
...
export default function HandSimulatorApp() {
  ...
  function buildHandRig(d) {
    ...
    // ── Polegar ───────────────────────────────────────────────────────────────
    // A palma local: X = comprimento (proximal→distal), Y = espessura (dorso→palmar), Z = largura (ulnar→radial)
    // O polegar sai do lado radial (Z positivo) da palma, base proximal.
    // Orientação anatômica do metacarpo do polegar em repouso:
    //   - sai da palma obliquamente ~40° em relação ao plano palmar (abdução palmar de repouso)
    //   - rotacionado ~90° axialmente (polpa virada para os outros dedos)
    // Usamos um grupo "thumbOrigin" com rotação estática que define a orientação de repouso,
    // e sobre ele encadeamos os grupos de movimento.

    // Posição de base: borda radial-proximal da palma
    const thumbBase = new THREE.Group();
    thumbBase.position.set(d.thumbBase.x, d.thumbBase.y, d.thumbBase.z);
    palm.add(thumbBase);

    // Orientação anatômica de repouso do CMC (rotação estática, não é slider)
    // Ordem ZYX: primeiro gira axialmente (polpa para os dedos), depois inclina para fora da palma, depois vira radialmente
    const thumbOrient = new THREE.Group();
    thumbOrient.rotation.order = "ZYX";
    thumbOrient.rotation.y = -deg2rad(55);  // vira eixo X do metacarpo ~55° na direção +Z radial/distal
    thumbOrient.rotation.z = -deg2rad(30);  // inclina metacarpo ~30° para baixo (palmar/ventral)
    thumbOrient.rotation.x =  deg2rad(80);  // rotação axial: polpa voltada para os outros dedos
    thumbBase.add(thumbOrient);

    // Cadeia de rotações de movimento (sobre a orientação de repouso)
    // cmcAbd  → abdução/adução palmar (gira em torno de Y local = normal ao plano da palma)
    const cmcAbd   = new THREE.Group(); thumbOrient.add(cmcAbd);
    // cmcFlex → flexão/extensão CMC (gira em torno de Z local = eixo transversal)
    const cmcFlex  = new THREE.Group(); cmcAbd.add(cmcFlex);
    // cmcAxial → oposição/retroposição = rotação axial do metacarpo (em torno de X local)
  ...
  useEffect(() => {
    ...
    // Polegar — eixos corrigidos para nova orientação anatômica:
    // cmcAbd  rotaciona em torno de Y local (normal ao plano da palma = abdução/adução palmar)
    // cmcFlex rotaciona em torno de Z local (eixo transversal = flexão/extensão CMC)
    // cmcAxial rotaciona em torno de X local (eixo axial do metacarpo = oposição/retroposição)
    // tmcp    rotaciona em torno de Z local (flexão MCP)
    // tip_ip  rotaciona em torno de Z local (flexão/extensão IP)
    const t = rig.thumb;
  ...
  const presetFunctional = () => { setGlobalMode("functional"); setGrip(50); applyGlobalGrip(50,"functional"); setOpenPanel("global"); setActivePreset("functional"); };
  const presetNeutral    = () => { resetToRest();   setActivePreset("neutro"); };
  const presetZero       = () => { resetToNeutral(); setActivePreset("zero"); };
  ...
  const ThumbSection = () => ([
    ...
    <LabeledSlider key={c.k} label={c.lbl} min={c.min} max={c.max} value={thumb[c.k]}
      onChange={v => { setThumbVal(c.k,v); setActivePreset("none"); }}
      leftHint={c.L} rightHint={c.R}
      onHighlight={() => setDebugKey(c.dbk)} />
  )));
    ...
    <div className="w-full h-screen text-gray-900 flex overflow-hidden"
      ...
      <aside className="w-[420px] max-w-[45%] h-full border-r border-gray-200 p-5 overflow-y-auto">
        ...
        <div className="flex items-center gap-2 mb-4 flex-wrap">
          <button onClick={presetFunctional} className="px-3 py-1.5 rounded-xl border text-xs font-medium transition-all" style={activePreset==="functional" ? {backgroundColor:"var(--lmb-blue)",color:"#fff",borderColor:"var(--lmb-blue)"} : {borderColor:"var(--lmb-blue)",color:"var(--lmb-blue)"}}>Funcional</button>
          <button onClick={presetNeutral}    className="px-3 py-1.5 rounded-xl border text-xs font-medium transition-all" style={activePreset==="neutro" ? {backgroundColor:"var(--lmb-blue)",color:"#fff",borderColor:"var(--lmb-blue)"} : {borderColor:"var(--lmb-blue)",color:"var(--lmb-blue)"}}>Neutro</button>
          <button onClick={presetZero}       className="px-3 py-1.5 rounded-xl border text-xs font-medium transition-all" style={activePreset==="zero"   ? {backgroundColor:"var(--lmb-blue)",color:"#fff",borderColor:"var(--lmb-blue)"} : {borderColor:"var(--lmb-blue)",color:"var(--lmb-blue)"}}>Zero</button>
        </div>
        ...
        <AccordionItem id="global_d2d5" title="Controle global D2–D5" isOpen={openPanel==="global_d2d5"} onToggle={id=>setOpenPanel(openPanel===id?"none":id)}>
          ...
          {[
            ...
            <LabeledSlider key={k} label={`${k} D2–D5 (${r[0]}..+${r[1]})`} min={r[0]} max={r[1]} value={globalD2D5[k]}
              onChange={v=>{ updateGlobalD2D5(k,v); setActivePreset("none"); }} leftHint={lH} rightHint={rH}
              onHighlight={() => setDebugKey(dbk)}
          />
          ))}
        </AccordionItem>
        ...
        <AccordionItem id="wrist" title="Punho (Flex/Ext, Desvio)" isOpen={openPanel==="wrist"} onToggle={id=>setOpenPanel(openPanel===id?"none":id)}>
          <LabeledSlider label={`Flexão/Extensão (${RANGES.WRIST_FLEX[0]}..+${RANGES.WRIST_FLEX[1]})`} min={RANGES.WRIST_FLEX[0]} max={RANGES.WRIST_FLEX[1]} value={wrist.flex}
            onChange={v=>{ setWrist(w=>({...w,flex:v})); setActivePreset("none"); }} leftHint="Extensão (−)" rightHint="Flexão (+)"
            onHighlight={() => setDebugKey('WR_FLEX')}/>
          <LabeledSlider label={`Desvio radial/ulnar (${RANGES.WRIST_DEV[0]}..+${RANGES.WRIST_DEV[1]})`} min={RANGES.WRIST_DEV[0]} max={RANGES.WRIST_DEV[1]} value={wrist.dev}
            onChange={v=>{ setWrist(w=>({...w,dev:v})); setActivePreset("none"); }} leftHint="Ulnar (−)" rightHint="Radial (+)"
            onHighlight={() => setDebugKey('WR_DEV')}/>
        </AccordionItem>
